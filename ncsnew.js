        ...
        .send({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(`** [${song.name}](https://discord.gg/vZnE8Dwx8t) **`)
              .addFields([
                {
                  name: `Requested By`, value: `${song.user} [${song.formattedDuration}]`,
                  inline: true,
                },
              ])
          ],
      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`Song has ended! Please Add Some More Trcks`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
          ...
          case "autoplay":
            {
              if (!channel) {
                return send(
                  interaction,
                  `${client.config.emoji.ERROR} You must be in a voice channel to use this command.`
                );
              } else if (
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} You must be in the same voice channel.`
                );
              } else if (!queue) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} Nothing's playing right now.`
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                  `${client.config.emoji.ERROR} You are not DJ and also you are not song requester.`
                );
              } else if (!queue.autoplay) {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Autoplay is now **Enabled**`
                );
              } else {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Autoplay is now **Disabled**`
                );
              }
            }
            break;
          case "skip":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} Current Track **Skiped**`
                );
              }
            }
            break;
          case "stop":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Music **Stoped**`
                );
              }
            }
            break;
          case "pauseresume":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Current Track **Resumed**`
                );
              } else if (!queue.paused) {
                await queue.pause();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Current Track **Paused**`
                );
              }
            }
            break;
          case "loop":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} Song Loop is now **Enabled**`
                );
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} Song Loop is now **Disabled**`
                );
              }
            }
            break;
      ...
      if (message.channelId === musicchannel.id) {
        ...
        if (message.author.bot) {
          ...
          if (
            ...
            return client.embed(
              message,
              `${client.config.emoji.ERROR} Your voice channel is full, I can't join.`
            );
